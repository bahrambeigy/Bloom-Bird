...
/* to avoid bloom arrays overflow :) */
#define BLOOM_TYPE u8
...
struct fib {
  ...
  /* specific bloom-bird variables */
  volatile u32 checksum;        /* checksum of bloom container */
  volatile BLOOM_TYPE *bloom_filter;    /* main bloom filter array */
  int bloom_filter_size;		/* size of the bloom */
  int bloom_num_hashes;			/* number of hashes used in bloom filter */
  int bloom_shift;				/* how many shift */
  int bloom_hash_order;			/* bloom hash order */
  int bloom_max_entries;
  int bloom_min_entries;
  volatile u8 bloom_feature_activated;   /* check if bloom feature should be activated or not */
};
