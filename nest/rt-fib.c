...
#define BLOOM_DEF_ORDER 16    /* default bloom order */
#define BLOOM_STEP 2
...
fib_init(struct fib *f, pool *p, unsigned node_size, unsigned hash_order, fib_init_func init)
{
  ...
  f->bloom_feature_activated = 0;
  
  if(!f->bloom_hash_order)
    f->bloom_hash_order = BLOOM_DEF_ORDER;
  
  f->bloom_max_entries = 1<<f->bloom_hash_order;
  f->bloom_min_entries = 1<<(f->bloom_hash_order-HASH_LO_STEP);
...
static void
fib_bloom_scale(struct fib *f, int step)
{
  if(f->bloom_hash_order >= BLOOM_DEF_ORDER)
    f->bloom_hash_order += step;
  
  if(!f->bloom_feature_activated)
  {
    f->bloom_feature_activated = 1;
    //DBG("Activating Bloom Feature\n");
  }
  else
    mb_free(f->bloom_filter);
      
  f->bloom_shift = (32 - (f->bloom_hash_order+step));
  f->bloom_filter_size = 1 << (f->bloom_hash_order+step);
  f->bloom_num_hashes = 3; // static num hashes ;)

  f->bloom_max_entries = 1<<f->bloom_hash_order;
  f->bloom_min_entries = 1<<(f->bloom_hash_order-step);

  // memory allocation of bloom filter array
  f->bloom_filter = mb_alloc(f->fib_pool, f->bloom_filter_size * sizeof(BLOOM_TYPE));
  bzero(f->bloom_filter, f->bloom_filter_size * sizeof(BLOOM_TYPE));

  //printf("[Bloom] order %d and Bloom filter size is %d [MAX] is %ld and [MIN] is %ld\n", f->bloom_hash_order, f->bloom_filter_size, f->bloom_max_entries, f->bloom_min_entries);

  // adding prefixes into bloom
  FIB_WALK(f, e)
  {
    f->bloom_filter[ipa_bloom_hash1(e->prefix, f->bloom_shift)]++;
    f->bloom_filter[ipa_bloom_hash2(e->prefix, f->bloom_shift)]++;
  }
  FIB_WALK_END;
}
...
fib_find(struct fib *f, ip_addr *a, int len)
{
  if(f->bloom_feature_activated)
  {
    if (!f->bloom_filter[ipa_bloom_hash1(*a, f->bloom_shift)])
      return NULL;
    if(!f->bloom_filter[ipa_bloom_hash2(*a, f->bloom_shift)])
      return NULL;
  }
...
fib_get(struct fib *f, ip_addr *a, int len)
{
  ...
  if(f->bloom_feature_activated)
  {
    if (f->bloom_filter[ipa_bloom_hash1(*a, f->bloom_shift)] && f->bloom_filter[ipa_bloom_hash2(*a, f->bloom_shift)])
      check_chains = 1;
  }
  
  if(check_chains || !f->bloom_feature_activated)
  {
    while (e && (e->pxlen != len || !ipa_equal(*a, e->prefix)))
      e = e->next;
  ...
  *ee = e;
  ...
  if(f->bloom_feature_activated)
  {
    f->bloom_filter[ipa_bloom_hash1(*a, f->bloom_shift)]++;
    f->bloom_filter[ipa_bloom_hash2(*a, f->bloom_shift)]++;
  }
  ...
  if(f->entries > f->bloom_max_entries)
      fib_bloom_scale(f, BLOOM_STEP);
      ...
      if (*ee == e)
	{
	  *ee = e->next;
	  if (it = e->readers)
	    ...
	    if(f->checksum=f->bloom_filter[ipa_bloom_hash1(e->prefix, f->bloom_shift)])
		  f->bloom_filter[f->checksum]--;
	    if(f->checksum=f->bloom_filter[ipa_bloom_hash2(e->prefix, f->bloom_shift)])
		  f->bloom_filter[f->checksum]--;
	  ...
	  if(f->entries < f->bloom_min_entries)
	    fib_bloom_scale(f, -BLOOM_STEP);
