...
#define ipa_bloom_hash1(x, y) ipv6_bloom_hash1(&(x), y)
#define ipa_bloom_hash2(x, y) ipv6_bloom_hash2(&(x), y)
...
static inline u32 ipv6_bloom_hash1(ip_addr *a, unsigned shift)
{
  u32 x = _I0(*a) ^ _I1(*a) ^ _I2(*a) ^ _I3(*a);
  return (x ^ (x<<8) ^ (x<<4)) >> shift;
}

static inline u32 ipv6_bloom_hash2(ip_addr *a, unsigned shift)
{
  u32 x = _I0(*a) ^ _I1(*a) ^ _I2(*a) ^ _I3(*a);
  return (x ^ (x<<10) ^ (x<<5)) >> shift;
}
