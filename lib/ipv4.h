...
#define ipa_bloom_hash1(x, y) ipv4_bloom_hash1(_I(x), y)
#define ipa_bloom_hash2(x, y) ipv4_bloom_hash2(_I(x), y)
...
static inline u32 ipv4_bloom_hash1(u32 a, unsigned shift)
{
  return (((a^0xdeadbeef) ^ (a<<4))) >> shift;
}

static inline u32 ipv4_bloom_hash2(u32 a, unsigned shift)
{
  return (((a+0x17bea992) ^ (a>>10))) >> shift;
}
